...
@section('content')
<div class="max-w-3xl">
    <h1 class="text-4xl font-semibold tracking-normal font-display mb-8">Edit Photo</h1>
    ...
    <form action="{{ route('admin.photos.update', $photo) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description"
                      name="description"
                      rows="4"
                      class="w-full bg-gray-900 border border-gray-700 px-4 py-3 text-white focus:border-gray-500 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description', $photo->description) }}</textarea>
            @error('description')
                <p class="text-red-400 text-sm mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        @if($photo->image_path)
            <div>
                ...
                <img src="{{ asset('storage/' . $photo->image_path) }}" alt="{{ $photo->title }}" class="w-48 h-48 object-cover border border-gray-700">
            </div>
        @endif

        <div>
            <label for="image" class="block text-sm text-gray-400 mb-2">{{ $photo->image_path ? 'Replace Image' : 'Image' }}</label>
            <input type="file"
                   id="image"
                   name="image"
                   accept="image/*"
        ...
        <div class="flex items-center">
            <input type="checkbox"
                   ...
                   name="featured"
                   value="1"
                   {{ old('featured', $photo->featured) ? 'checked' : '' }}
