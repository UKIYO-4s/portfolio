@extends('layouts.app')

@section('title', $project->title . ' - Projects')

@section('content')
<article class="py-24 px-6 sm:px-8 lg:px-12">
    <div class="max-w-5xl mx-auto">
        <div class="mb-12">
            <a href="{{ route('projects.index') }}" class="inline-flex items-center text-sm text-gray-400 hover:text-white transition-colors mb-8">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 19l-7-7 7-7"></path>
                </svg>
                Back to Projects
            </a>

            <h1 class="text-5xl md:text-6xl font-thin tracking-wide mb-6">{{ $project->title }}</h1>

            @if($project->technologies)
                <div class="flex flex-wrap gap-2 mb-8">
                    @foreach($project->technologies as $tech)
                        <span class="text-xs px-3 py-1 border border-gray-700 text-gray-400">
                            {{ $tech }}
                        </span>
                    @endforeach
                </div>
            @endif

            <p class="text-xl text-gray-400 font-light leading-relaxed">
                {{ $project->description }}
            </p>
        </div>

        @if($project->thumbnail)
            <div class="aspect-[16/9] bg-gray-900 mb-12 overflow-hidden">
                <img src="{{ asset('storage/' . $project->thumbnail) }}"
                     alt="{{ $project->title }}"
                     class="w-full h-full object-cover">
            </div>
        @endif

        @if($project->images)
            <div class="space-y-12 mb-16">
                @foreach($project->images as $image)
                    <div class="aspect-[16/9] bg-gray-900 overflow-hidden">
                        <img src="{{ asset('storage/' . $image) }}"
                             alt="{{ $project->title }}"
                             class="w-full h-full object-cover">
                    </div>
                @endforeach
            </div>
        @endif

        <div class="flex flex-wrap gap-6 pt-12 border-t border-gray-800">
            @if($project->url)
                <a href="{{ $project->url }}"
                   target="_blank"
                   rel="noopener noreferrer"
                   class="inline-flex items-center px-8 py-3 border border-white hover:bg-white hover:text-black transition-all duration-300 text-sm tracking-wider">
                    View Live
                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                    </svg>
                </a>
            @endif

            @if($project->github_url)
                <a href="{{ $project->github_url }}"
                   target="_blank"
                   rel="noopener noreferrer"
                   class="inline-flex items-center px-8 py-3 bg-gray-800 hover:bg-gray-700 transition-all duration-300 text-sm tracking-wider">
                    View Code
                    <svg class="w-4 h-4 ml-2" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                    </svg>
                </a>
            @endif
        </div>
    </div>
</article>
@endsection
